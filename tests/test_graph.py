...
def test_review_retry_routing():
    ...
    assert after_review_qc({"review_qc": {"passed": False}, "retry_count_qc": 99}) == "writer"
    assert after_review_qc({"review_qc": {"passed": True}, "mode": "qc_only"}) == "writer"
    assert (
        after_review_qc(
            {
    ...
    assert after_review_qc({"review_qc": {"passed": True}, "interrupt_after_qc": True, "auto_confirm": False}) == "writer"
    assert after_review_down({"review_downstream": {"passed": False}, "retry_count_downstream": 0}) == "retry"
    assert after_review_down({"review_downstream": {"passed": True}, "retry_count_downstream": 0}) == "writer"
...
def test_r_language_degrades(tmp_path):
    state = run_analysis("seurat 分析", data_path="", tissue="pbmc", language="r", execute_code=False)
