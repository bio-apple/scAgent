...
Do not cluster on UMAP. Exploratory cluster markers (Wilcoxon / t-test / MAST via rank_genes on leiden) are not group-level DE. Condition comparisons must call pseudobulk_de (sample × cell type; engine auto uses edgeR then DESeq2 via rpy2, else t-test+BH). **Hard rule:** when condition_key is set and n_replicates≥2, cell-level Wilcoxon/MAST on the condition column is forbidden; only pseudobulk + DESeq2/edgeR (+ FDR) for between-group conclusions. Honor DESeq2/edgeR/t-test/MAST if named in the task. Cross-validate with a second method; report overlap. Never treat cell-level MAST/Wilcoxon as the between-condition result.
Annotation ensemble: after CellTypist, call ensemble_cell_annotation; write final labels to obs['scagent_annotation'] (scANVI fallback when max_prob < 0.8).
