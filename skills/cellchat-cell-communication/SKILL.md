...
> **Upstream**: [SciAgent-Skills/cellchat-cell-communication](https://github.com/jaechang-hits/SciAgent-Skills/tree/main/skills/systems-biology-multiomics/cellchat-cell-communication). R/Seurat workflow; scAgent 默认可执行路径仍是 Scanpy/AnnData — 导出 `adata_processed.h5ad` 后按本 skill 写 Rmd，或任务中指定 `--language r` 的双重格式块。
...
- `single-cell-annotation-guide` / `celltypist-cell-annotation` — accurate `cell_type` labels are required before interpreting sender/receiver roles
- `scanpy-scrna-seq` — QC → clustering pipeline that produces the annotated AnnData exported to Seurat/CellChat
- `harmony-batch-correction` — integrate multi-sample data before communication analysis when batches drive clustering
- `single-cell-annotation` — sc-best-practices annotation decision tree (prerequisite quality for LR inference)
